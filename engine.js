...
/* ── [NOYAU] Canvas ─────────────────────────────────────────────── */
const TILE = 16;
const VIEW_TILES_W = 28;            // ~448px de large visible
const VIEW_TILES_H = 16;            // 256px de haut (16 tuiles)
const W = VIEW_TILES_W * TILE;      // 448 — largeur logique (viewport)
const H = VIEW_TILES_H * TILE;      // 256 — hauteur logique
const canvas = document.getElementById('c');
const ctx = canvas.getContext('2d');
ctx.imageSmoothingEnabled = false;
let S = 1;
function resize(){
  const uw = window.innerWidth - 16;
  const uh = window.innerHeight - 70;
  let cw = uw, ch = Math.round(uw * H / W);
  if(ch > uh){ ch = uh; cw = Math.round(uh * W / H); }
...
function triggerDash(){
  ...
  const up=keys['ArrowUp']||keys['Space'], down=keys['ArrowDown']||keys['KeyS'];
  dash.dx = right?1:left?-1:p.facing;
  dash.dy = up?-1:down?0.6:0;
  const len=Math.sqrt(dash.dx*dash.dx+dash.dy*dash.dy)||1;
  dash.dx/=len; dash.dy/=len;
...
function showEnd(timeMs){
  let txt = 'Pi\u00e8ces : '+coinCount+' / 3';
  if(secretCoinTaken) txt += '  \u2022  Pi\u00e8ce secr\u00e8te obtenue !';
  document.getElementById('end-coins').textContent = txt;
  const te = document.getElementById('end-time');
  if(te) te.textContent = 'Temps : ' + formatTime(timeMs);
...
function updatePlayer(){
  ...
  if(dash.active){
    ...
    for(const t of dash.trail) t.a*=0.82;
    updatePowerTimers();
...
function drawWorld(){
  ...
  for(const c of coins){
    ...
    if(c.cond && c.cond.indexOf('aerien')>=0 && !c._unlocked) continue;
    const t=Date.now()/150, ph=Math.abs(Math.sin(t)), cw=Math.max(3,c.w*ph);
    ctx.fillStyle='#ffcc22'; ctx.fillRect(c.x+(c.w-cw)/2,c.y,cw,c.h);
    ctx.fillStyle='#fff0a0'; ctx.fillRect(c.x+(c.w-cw)/2,c.y+2,Math.max(1,cw*0.3),c.h-4);
    ctx.fillStyle='#cc9810'; ctx.fillRect(c.x+(c.w-cw)/2,c.y+c.h-2,cw,2);
  }
...
function drawPlayer(){
  ...
  for(const b of balls){ ctx.fillStyle='#5a2808'; ctx.fillRect(Math.round(b.x-b.r),Math.round(b.y-b.r),b.r*2,b.r*2);
    ctx.fillStyle='#ff7722'; ctx.fillRect(Math.round(b.x-b.r+1),Math.round(b.y-b.r+1),b.r*2-2,b.r*2-2); }
  // aura géant (Victor)
...
function drawHUD(){
  ...
  ctx.fillStyle='#ffcc22'; ctx.fillRect(12,12,8,10);
  ctx.fillStyle='#fff0a0'; ctx.fillRect(13,13,2,8);
  ctx.fillStyle='#fff'; ctx.font='bold 12px monospace';
  ctx.fillText('\u00d7 '+coinCount+' / 3', 26, 21);
  ...
  // chrono du run (haut-centre)
  const tStr = formatTime(Math.round(runFrames/FPS*1000));
  ctx.font='bold 16px monospace';
  const tw = ctx.measureText(tStr).width;
  ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.fillRect(W/2-tw/2-8, 6, tw+16, 24);
  ctx.fillStyle='#ffe680'; ctx.fillText(tStr, W/2-tw/2, 23);
}
