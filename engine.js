...
// Appui sur POUVOIR (F / bouton tactile) : pouvoir du perso uniquement.
// (Le lancer de caleçon a désormais SON PROPRE bouton, voir doCaleconPress.)
function doPowerPress(){
  if(p.dead) return;
  if(charDef.hasDash && !dash.active && dash.cooldown===0){ triggerDash(); if(window.Audio2) Audio2.sfx('throw'); }
...
// Appui sur CALEÇON (bouton tactile dédié / clavier) : lance un caleçon si le
// joueur en porte un et qu'un boss est présent.
function doCaleconPress(){
  if(p.dead) return;
  if(boss) caleconAction();
}
// Nombre de caleçons actuellement portés (lu par l'UI tactile pour afficher le bouton).
function caleconCount(){ return (typeof carriedCalecons==='number') ? carriedCalecons : 0; }
// Y a-t-il un combat de boss en cours (niveau 3) ? (lu par l'UI tactile)
function bossActive(){ return !!(boss && boss.alive); }
// Direction tactile : dx (-1 gauche, 1 droite, 0), dy (1 bas, 0). Le bas accroupit.
function setTouchDir(dx, dy){
  keys['ArrowLeft']  = (dx<0);
  keys['ArrowRight'] = (dx>0);
  keys['ArrowDown']  = (dy>0);
}
// expose pour la couche tactile (index.html)
window.CCB_input = { jumpPress:doJumpPress, jumpRelease:doJumpRelease, power:doPowerPress,
                     calecon:doCaleconPress, caleconCount:caleconCount, bossActive:bossActive,
                     dir:setTouchDir,
...
window.addEventListener('keydown', e => {
  ...
  // KeyF : action contextuelle au clavier — lance un caleçon si un boss est là
  // (comportement historique), sinon déclenche le pouvoir du perso.
  if(e.code==='KeyF'){ if(boss) doCaleconPress(); else doPowerPress(); }
  // KeyE : bouton pouvoir explicite (utile si on veut le pouvoir même face au boss).
  if(e.code==='KeyE'){ doPowerPress(); }
  keys[e.code] = true;
    ...
    for(const el of z.elements){
      ...
      switch(el.type){
        ...
        case 'boss_louise': {
          ...
          // Boss 20% plus grand que le rectangle de la map, pieds ancrés au sol :
          // on augmente w/h de 20% et on décale x/y pour garder le bas et le centre.
          const gw = r.w*1.2, gh = r.h*1.2;
          const gx = r.x - (gw - r.w)/2;      // élargissement centré horizontalement
          const gy = r.y - (gh - r.h);        // le bas reste au même niveau (pieds au sol)
          boss = {
            x:gx, y:gy, w:gw, h:gh,
...
// Vérifie qu'un rectangle (x,y,w,h) donné est libre de tout solide bloquant.
// Sert à valider qu'on peut occuper une nouvelle boîte (ex: se redresser) sans
// s'encastrer dans un mur/plafond — ce qui provoquerait une éjection latérale
// brutale par collideSolidsX au tick suivant (le bug de "téléportation").
function boxClear(x, y, w, h){
  for(const s of solids){
    if(!gateBlocks(s)) continue;
    if(aabb(x, y, w, h, s.x, s.y, s.w, s.h)) return false;
  }
  for(const b of breakables){
    if(!b.alive) continue;
    if(aabb(x, y, w, h, b.x, b.y, b.w, b.h)) return false;
  }
  for(const fb of fragileBlocks){
    if(!fb.alive) continue;
    if(aabb(x, y, w, h, fb.x, fb.y, fb.w, fb.h)) return false;
  }
  return true;
}

// Vérifie qu'il y a la place de se redresser : teste si la bande de hauteur `dH`
// située juste au-dessus du joueur (à la position verticale `topY`) est libre de
// tout solide bloquant. Sert à éviter de traverser un plafond bas en quittant
// l'accroupissement en l'air.
function headroomClear(topY, dH){
  if(dH<=0) return true;
  const x=p.x, w=p.w;
  for(const s of solids){
    if(!gateBlocks(s)) continue;
    if(aabb(x, topY, w, dH, s.x, s.y, s.w, s.h)) return false;
  }
  for(const b of breakables){
    if(!b.alive) continue;
    if(aabb(x, topY, w, dH, b.x, b.y, b.w, b.h)) return false;
  }
  for(const fb of fragileBlocks){
    if(!fb.alive) continue;
    if(aabb(x, topY, w, dH, fb.x, fb.y, fb.w, fb.h)) return false;
  }
  return true;
}
...
function showEnd(timeMs){
  ...
  if(dd){
    ...
    if(secretCoinTaken) html += '<img src="duck.svg" alt="" class="got" title="Canard rare (vaut 2 canards)" style="filter:hue-rotate(135deg) saturate(1.4) drop-shadow(0 0 7px #1fd6c6)">';
    dd.innerHTML = html;
  }
...
function updatePlayer(){
  ...
  // Règle : on ne DÉCLENCHE l'accroupissement qu'au sol, mais on le MAINTIENT en
  // l'air tant que le joueur garde le bas enfoncé. Cela permet de sauter en restant
  // accroupi (saut accroupi) au lieu de forcer un redressement au décollage.
  const canCrouch = charDef.canCrouch!==false && !giant.active;
  if(dn && canCrouch && (p.onGround || p.crouching)){
    // s'accroupir (déclenchement au sol) ou rester accroupi (au sol comme en l'air)
    if(!p.crouching){ p.crouching=true; p.y+=(PLAYER_H-PLAYER_H_CR); p.h=PLAYER_H_CR; }
    if(p.onGround) p.vx *= 0.80;   // friction d'accroupi seulement au sol
  } else if(p.crouching){
    // le joueur relâche le bas (ou perd la possibilité de s'accroupir) : on se
    // redresse. On vérifie que la boîte DEBOUT complète (à sa position finale) est
    // libre — pas seulement la bande du dessus — sinon grandir encastrerait le
    // joueur dans un mur adjacent et collideSolidsX l'éjecterait latéralement à
    // grande vitesse (le bug de téléportation gauche/droite). Si ce n'est pas
    // libre, on reste accroupi ce tick.
    const standH = (giant.active?Math.round(charDef.drawH*1.5):PLAYER_H);
    const dH = standH - p.h;                 // hauteur à regagner vers le haut
    if(dH<=0 || boxClear(p.x, p.y - dH, p.w, standH)){
      p.crouching=false; p.y-=dH; p.h=standH;
    }
    // sinon : pas la place, on reste accroupi ce tick (pas de changement)
  }
  // Horizontal
  if(p.onGround){
  ...
  if(p.jumpBuf>0 && p.coyote>0){ p.vy=PHYS.jumpForce; p.coyote=0; p.jumpBuf=0;
    // On ne force PLUS le redressement ici. Si le joueur maintient le bas, il
    // saute en restant accroupi ; s'il ne le maintient pas, le bloc d'accroupissement
    // ci-dessus le redressera proprement au tick suivant (avec correction de p.y).
    // Auparavant, on remettait crouching=false et h=PLAYER_H SANS corriger p.y, ce
    // qui enfonçait le personnage dans le sol de (PLAYER_H - PLAYER_H_CR) et
    // provoquait une ré-éjection brutale par la collision verticale (le "recul rapide").
  }
...
function updateBoss(){
  ...
  if(boss.alive){
    ...
    // --- CONTACT DIRECT avec Louise : touche le joueur = mort ---
    // (Louise se blesse uniquement au caleçon ; tout contact corporel est mortel.)
    if(!p.dead && aabb(p.x,p.y,p.w,p.h, boss.x,boss.y,boss.w,boss.h)){ killPlayer(); return; }
  }
