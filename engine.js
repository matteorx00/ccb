...
/* Lancer un caleçon — déclenché par la touche d'action (KeyF).
   Trajectoire en ARC parabolique, identique à la boule de feu de Mattéo.
   Le RAMASSAGE est automatique au contact (voir pickupCalecons). */
...
function defeatBoss(){
  ...
  if(bossWall){
    // ouverture ANIMÉE : le mur s'enfonce dans le sol sur ~36 frames, puis devient franchissable.
...
function drawWorld(){
  ...
  for(const s of solids){
    ...
    if(s.secretWall){
      ...
      ctx.fillStyle=P.top;  ctx.fillRect(s.x,s.y,s.w,3);
      ctx.fillStyle=B.bottom; ctx.fillRect(s.x,s.y+s.h-2,s.w,2);
      ctx.fillStyle=B.top;
