...
function buildLevel(){
  ...
  lava=[]; retractSpikes=[]; geysers=[]; sinkers=[]; pistons=[]; fragileBlocks=[];
...
function collideSolidsY(){
  ...
  if(THEME.ceiling && p.vy < 0){
    // profondeur locale de la voûte au-dessus du joueur (segments ceilingLow)
    let d = THEME.ceiling.depth;
    const px = p.x + p.w/2;
    for(const s of (LEVEL.ceilingLow||[])){
      if(px >= s.x*TILE && px < (s.x+s.w)*TILE){ d = Math.max(d, s.depth*TILE); break; }
    }
    // barrière à la partie la plus basse de la roche locale (laisse passer dessous)
    const ceilBottom = d * 0.82;
    if(p.y < ceilBottom){ p.y = ceilBottom; p.vy = 0; }
  }
...
function updateBoss(){
  ...
  // --- MUR DE SORTIE : solide tant que Louise vit ---
  if(bossWall && !bossWall.open){
    // le mur agit comme un solide : on l'ajoute dynamiquement à la détection
    // (géré dans collideSolidsY via bossWallSolid()). Rien à faire ici.
  }
...
/* Ramassage / lancer de caleçon — déclenché par la touche d'action (KeyF). */
function caleconAction(){
  if(!boss) return;
  // 1) si à portée d'un caleçon non pris -> ramasser
  for(const c of calecons){
    if(c.taken) continue;
    const cx=c.x+c.w/2, cy=c.y+c.h/2, px=p.x+p.w/2, py=p.y+p.h/2;
    if(Math.abs(cx-px)<=CALECON_RANGE && Math.abs(cy-py)<=CALECON_RANGE){
      c.taken=true; carriedCalecons++; return;
    }
  }
  // 2) sinon, si on porte au moins un caleçon -> lancer
...
/* Défaite du boss : ouvre le mur, fait apparaître le canard du boss. */
function defeatBoss(){
  boss.alive=false; bossDefeated=true;
  if(bossWall){
    bossWall.open=true;
    const i=solids.indexOf(bossWall);
    if(i>=0) solids.splice(i,1);   // retire le mur -> sortie ouverte
  }
...
function drawBg(){
  ...
  if(THEME.ceiling){
    ...
    // Profondeur LOCALE de la voûte : par défaut 'base', mais augmentée sur les
    // segments de plafond bas (LEVEL.ceilingLow : [{x,w,depth}] en tuiles).
    // Conversion : à une position écran sx, on calcule la profondeur cible en px.
    const lows = (LEVEL.ceilingLow||[]).map(s=>({x0:s.x*TILE, x1:(s.x+s.w)*TILE, d:s.depth*TILE}));
    function depthAtWorld(wx){
      let d = base;
      for(const s of lows){ if(wx>=s.x && wx<s.x1){ d = Math.max(d, s.d); break; } }
      return d;
    }
    const STEP=18;
    function peakY(cell){
      const wx = cell*STEP;
      const d = depthAtWorld(wx);
      // dentelure proportionnelle à la profondeur locale
      return d*0.55 + hash(cell)*d*0.30 + hash(cell*7+3)*d*0.18;
    }
    const c0=Math.floor(camX/STEP)-1, c1=Math.floor((camX+W)/STEP)+1;
    const pts=[];
    for(let c=c0;c<=c1;c++){ pts.push([c*STEP-camX, peakY(c)]); }
    // masse profonde
    ...
    // STALACTITES massives, sous certains creux, largeur réelle
    for(let c=c0;c<=c1;c++){
      const r=hash(c*3+99);
      if(r<0.5) continue;
      const x = c*STEP - camX + (hash(c+11)*8-4);
      const baseY = peakY(c)*0.82;
      const dloc = depthAtWorld(c*STEP);
      const len = dloc*0.5 + r*dloc*0.9;
      const w = 7 + (r*8|0);
      ctx.fillStyle=C.rockDeep;
      ctx.beginPath();
    ...
    for(let c=c0*2;c<=c1*2;c++){
      const r=hash(c*5+555); const x=c*(STEP/2)-camX;
      const yMax=peakY(Math.floor(c/2))*0.7; const gy=4+r*yMax;
...
function drawHUD(){
  ...
  // ── BARRE DE VIE DU BOSS (centrée en haut) ──
  if(boss){
