  ...
  for(const z of LEVEL.zones){
    for(const el of z.elements){
      ...
      switch(el.type){
        ...
        case 'sol':
        case 'bloc_solide':
          solids.push({...r, ice: !!el.ice, stone: !!el.stone}); break;
        case 'tuyau':
...
function updateBoss(){
  ...
  // --- CALEÇONS en vol (lancés par le joueur) : touchent Louise ---
  for(const s of caleconShots){ s.x+=s.vx; s.dist+=Math.abs(s.vx); }
  for(let i=caleconShots.length-1;i>=0;i--){
    ...
    // dépasse sa portée (3 cases) ou sort de l'écran -> disparaît (raté)
    if(s.dist>=CALECON_THROW_DIST || s.x<0 || s.x>LEVEL.longueur*TILE){ caleconShots.splice(i,1); }
  }
...
function caleconAction(){
  ...
  if(carriedCalecons>0){
    ...
    caleconShots.push({x:p.x+p.w/2, y:p.y+p.h*0.4, w:14, h:10, vx:dir*4.0, dist:0});
  }
}
