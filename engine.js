...
function collideSolidsY(){
  ...
  if(THEME.ceiling && p.vy < 0){
    const ceilBottom = THEME.ceiling.depth * 0.4;   // px depuis le haut de l'écran
...
function checkLevelInteractions(){
  ...
  // lave — mortelle pour TOUT LE MONDE (immunité aux pics non applicable).
  // Mort dès que le bas du joueur franchit la surface de la nappe : il n'y a
  // aucun solide sous la lave, donc on ne peut pas "marcher dessus".
...
function drawBg(){
  ...
  if(THEME.ceiling){
    ...
    // Sommets de la dentelure rocheuse, espacés irrégulièrement, reliés par des
    // segments OBLIQUES -> aspect minéral cassé, pas urbain.
    // On génère les pics sur une grille de ~18px, hauteur pseudo-aléatoire.
    const STEP=18;
    function peakY(cell){
      return base*0.25 + hash(cell)*base*0.45 + hash(cell*7+3)*base*0.28;
    }
    // construit la polyligne du bord (en coords écran), pics tous les STEP
    ...
    // couche claire (relief) : mêmes pics, remontés, légèrement décalés
    ctx.fillStyle=C.rock;
    ctx.beginPath(); ctx.moveTo(pts[0][0],0);
    for(const p of pts) ctx.lineTo(p[0], p[1]*0.78);
    ctx.lineTo(pts[pts.length-1][0],0); ctx.closePath(); ctx.fill();
    ...
    for(let c=c0;c<=c1;c++){
      ...
      const baseY = peakY(c)*0.78;
      const len = base*0.7 + r*base*1.6;
      const w = 7 + (r*8|0);                          // base large (7..15 px)
      ctx.fillStyle=C.rockDeep;
      ctx.beginPath();
      ctx.moveTo(x-w/2, baseY); ctx.lineTo(x, baseY+len); ctx.lineTo(x+w/2, baseY); ctx.closePath(); ctx.fill();
      ctx.fillStyle=C.rock;                           // facette éclairée (gauche)
      ctx.beginPath();
