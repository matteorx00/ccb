...
window.addEventListener('keydown', e => {
  ...
  if(e.code==='KeyF' && !p.dead){
    ...
    else if(charDef.hasDash && !dash.active && dash.cooldown===0) triggerDash();
...
let bossArenaX = Infinity;   // x (px) d'entrée de l'arène du boss
let bossEngaged = false;     // le joueur est-il entré dans l'arène ?
...
function buildLevel(){
  ...
  bossArenaX=Infinity; bossEngaged=false;
    ...
    for(const el of z.elements){
      ...
      switch(el.type){
        ...
        case 'boss_louise': {
          ...
          bossArenaX = z.x_debut * TILE;   // entrée de l'arène (px)
          boss = {
...
function collideSolidsY(){
  ...
  // stalactites : déclenchement quand le joueur passe DESSOUS (idle).
  // Pendant la chute, le SOMMET porte le joueur (plateforme) ; la pointe tue
  // (géré dans la détection de mort plus bas).
  for(const f of fallers){
    if(f.state==='gone') continue;
    // déclenchement : joueur sous la stalactite au repos
    if(f.state==='idle'){
      if(p.x+p.w > f.x && p.x < f.x+f.w && p.y > f.y){
        f.state='shaking'; f.timer=24;
      }
    }
    // sommet = plateforme : le joueur posé dessus descend avec elle (états falling/breaking)
    if((f.state==='falling'||f.state==='breaking') && p.vy>=0
...
function checkLevelInteractions(){
  ...
  // stalactites en chute : la POINTE (bas) tue ; le sommet est une plateforme.
  // Mort si la moitié basse de la stalactite chevauche le corps du joueur, SAUF
  // s'il est posé sur le sommet (tête au-dessus du haut de la stalactite).
  for(const f of fallers){
  ...
  // sceaux secrets (N3) : activation au contact. Les 3 ouvrent la porte de crypte.
  for(const sl of seals){
    if(!sl.active && aabb(p.x,p.y,p.w,p.h,sl.x,sl.y,sl.w,sl.h)){ sl.active=true; }
  }
...
function updateBoss(){
  if(!boss) return;
  // Le joueur est "engagé" dès qu'il franchit l'entrée de l'arène. La barre de
  // vie ne s'affiche qu'à partir de ce moment (et reste ensuite affichée).
  if(!bossEngaged && (p.x + p.w) >= bossArenaX) bossEngaged = true;

  pickupCalecons();   // ramassage automatique au contact

  // --- MUR DE SORTIE : animation d'ouverture (s'enfonce dans le sol) quand Louise meurt ---
  if(bossWall && bossWall.opening && !bossWall.open){
    bossWall.openT++;
    const k = Math.min(1, bossWall.openT / bossWall.openDur);
    // le mur se rétracte vers le bas : sa hauteur diminue, son sommet descend
  ...
  // --- CALEÇONS en vol (lancés par le joueur, trajectoire en arc) : touchent Louise ---
  ...
  for(let i=caleconShots.length-1;i>=0;i--){
    ...
    // fin de vie, sortie d'écran ou retombée dans la lave/sol -> disparaît
...
/* Lancer un caleçon — déclenché par la touche d'action (KeyF).
   Trajectoire en ARC parabolique, similaire à la boule de feu de Mattéo
   (vitesse horizontale + impulsion vers le haut, puis chute par gravité).
   Le RAMASSAGE est automatique au contact (voir pickupCalecons dans updateBoss). */
...
/* Ramassage AUTOMATIQUE au contact : tout caleçon non pris qui chevauche le
   joueur est ramassé immédiatement (pas besoin d'appuyer sur une touche). */
...
/* Défaite du boss : déclenche l'animation d'ouverture du mur + canard du boss. */
function defeatBoss(){
  boss.alive=false; bossDefeated=true;
  if(bossWall){
    // ouverture ANIMÉE : le mur s'enfonce dans le sol sur ~36 frames, puis devient
    // franchissable. On garde sa hauteur pleine au départ et on la réduit dans updateBoss.
    bossWall.opening=true;
    bossWall.openT=0;
    bossWall.openDur=36;
    bossWall.fullH=bossWall.h;     // mémorise la hauteur initiale (px)
...
function drawWorld(){
  ...
  for(const s of solids){
    ...
    if(s.secretWall){
      ...
      // DALLE pleine texturée comme une plateforme (mêmes teintes marron du thème).
      const P=THEME.platform, B=THEME.block;
      ctx.fillStyle=B.face; ctx.fillRect(s.x,s.y,s.w,s.h);          // corps marron foncé du thème
      ctx.fillStyle=P.top;  ctx.fillRect(s.x,s.y,s.w,3);            // arête supérieure (marron chaud, comme les plateformes)
      ctx.fillStyle=B.bottom; ctx.fillRect(s.x,s.y+s.h-2,s.w,2);    // base sombre
      // motif de briques discret (cohérent avec les autres blocs du thème)
      ctx.fillStyle=B.top;
      for(let ry=s.y+3; ry<s.y+s.h-2; ry+=8){
        const off=(((ry-s.y)/8)|0)%2?8:0;
        for(let rx=s.x-off; rx<s.x+s.w; rx+=16)
          ctx.fillRect(Math.max(s.x,rx)+1,ry+1,Math.min(14,s.x+s.w-Math.max(s.x,rx))-2,5);
      }
  ...
  for(const sl of seals){
    ...
    if(sl.active){
      ...
      ctx.strokeStyle='#6a5060'; ctx.lineWidth=1;
    }
  ...
  // porte de crypte : lourde dalle verrouillee tant que les sceaux manquent.
  if(cryptDoor && !cryptDoor.open && !(cryptDoor.x+cryptDoor.w<x0||cryptDoor.x>x1)){
    const d=cryptDoor;
    ctx.fillStyle='#241a1e'; ctx.fillRect(d.x,d.y,d.w,d.h);
    ctx.fillStyle='#3a2a30'; ctx.fillRect(d.x+2,d.y+2,d.w-4,d.h-4);
    // ferrures horizontales
    ctx.fillStyle='#5a4048';
    for(let ry=d.y+6; ry<d.y+d.h-4; ry+=12) ctx.fillRect(d.x+2,ry,d.w-4,3);
    // nombre de sceaux actifs / total, en gemmes au centre
    const total=seals.length, on=seals.filter(s=>s.active).length;
    const gw=6, gap=4, totW=total*gw+(total-1)*gap, gx=d.x+(d.w-totW)/2, gy=d.y+d.h/2-3;
    for(let i=0;i<total;i++){
      ctx.fillStyle = i<on ? '#ffb84a' : '#3a2c30';
      ctx.fillRect(gx+i*(gw+gap), gy, gw, 6);
    }
    ctx.strokeStyle='#120a0c'; ctx.strokeRect(d.x+.5,d.y+.5,d.w-1,d.h-1);
  }
  // trampolines
  for(const t of trampolines){
  ...
  // joueur
  drawPlayer();
  // boss (Louise) : dessiné après le joueur, indépendamment de l'état de mort,
...
/* Dessin d'un caleçon/slip blanc, en silhouette reconnaissable, dessiné en
   FORME POSITIVE (on ne peint que le tissu) : large ceinture en haut, puis un
   corps qui se sépare en deux jambes courtes laissant voir le décor entre elles. */
function drawCalecon(x,y,w,h){
  const wb='#ffffff', sh='#dcdce4', el='#aeb6c8', el2='#cfd4e0', ink='#1a1a1a';
  const beltH=Math.max(3,Math.round(h*0.32));        // ceinture
  const legW=Math.max(3,Math.round(w*0.36));          // largeur d'une jambe
  const legTop=y+beltH;                               // les jambes partent sous la ceinture
  // ── ceinture élastique (large bande pleine en haut) ──
  ctx.fillStyle=el;  ctx.fillRect(x, y, w, beltH);
  ctx.fillStyle=el2; ctx.fillRect(x, y, w, Math.max(1,Math.round(beltH*0.5)));
  // ── les deux jambes (rectangles bas-gauche et bas-droit), trou central laissé vide ──
  ctx.fillStyle=wb;
  ctx.fillRect(x,          legTop, legW, h-beltH);    // jambe gauche
  ctx.fillRect(x+w-legW,   legTop, legW, h-beltH);    // jambe droite
  // léger pont de tissu qui relie les deux jambes juste sous la ceinture (entrejambe haute)
  ctx.fillRect(x, legTop, w, Math.max(2,Math.round((h-beltH)*0.35)));
  // ── ombrage (bas des jambes + entrejambe) ──
  ctx.fillStyle=sh;
  ctx.fillRect(x, y+h-2, legW, 2);
  ctx.fillRect(x+w-legW, y+h-2, legW, 2);
  // ── contour sombre lisible ──
  ctx.fillStyle=ink;
  ctx.fillRect(x, y, w, 1);                            // haut ceinture
  ctx.fillRect(x, y, 1, h); ctx.fillRect(x+w-1, y, 1, h);          // côtés extérieurs
  ctx.fillRect(x, y+h-1, legW, 1); ctx.fillRect(x+w-legW, y+h-1, legW, 1); // bas des 2 jambes
  // bords intérieurs des jambes (le V de l'entrejambe)
  const ent=legTop+Math.max(2,Math.round((h-beltH)*0.35));
  ctx.fillRect(x+legW-1,   ent, 1, h-beltH);          // bord int. jambe gauche
  ctx.fillRect(x+w-legW,   ent, 1, h-beltH);          // bord int. jambe droite
  ctx.fillRect(x+legW-1,   ent, w-2*legW+1, 1);       // base de l'entrejambe
}
...
function drawHUD(){
  ...
  // ── BARRE DE VIE DU BOSS ── n'apparaît qu'une fois entré dans l'arène.
  // Descendue (byb=46) pour passer SOUS la barre de menu (Recommencer / Personnages).
  if(boss && bossEngaged){
    const bw=Math.min(220, W*0.5), bx=(W-bw)/2, byb=46;
    // nom
...
/* ── [NOYAU] Boucle principale à PAS DE TEMPS FIXE (60 Hz logique) ──
   La simulation (update) avance par pas fixes de 1/60 s, quel que soit le taux
   de rafraîchissement de l'écran (60, 120, 144 Hz...). Le rendu se fait une fois
   par frame d'affichage. Sans cela, la vitesse du jeu suivait la fréquence de
   l'écran (trop rapide sur 120/144 Hz). */
const FIXED_DT = 1000/60;            // durée d'un pas logique (ms)
let _lastTime = 0;                   // horodatage de la frame précédente
let _accum = 0;                      // temps accumulé en attente de simulation
const _now = () => (typeof performance!=='undefined' && performance.now) ? performance.now() : Date.now();

function loop(){
  const t = _now();
  let frame = t - _lastTime;
  _lastTime = t;
  if(frame > 250) frame = 250;       // garde anti-"spirale de la mort" (onglet en arrière-plan)
  _accum += frame;
  // rattrapage : autant de pas logiques de 1/60 s que nécessaire
  let steps = 0;
  while(_accum >= FIXED_DT && steps < 5){
    update();
    _accum -= FIXED_DT;
    steps++;
  }
  if(steps === 5) _accum = 0;        // si on n'arrive pas à suivre, on abandonne le reliquat
...
function startGame(){
  ...
  placePlayer();
  runFrames = 0; runActive = true;
  _lastTime = _now(); _accum = 0;    // initialise l'horloge du pas fixe
  loop();
}
