...
window.Audio2 = (function(){
  ...
  let ctx = null;
  let master = null;          // gain général (réglage volume + on/off)
  let sfxGain = null;         // sous-bus des effets sonores
  let musicGain = null;       // sous-bus de la musique (fichiers <audio>)
  let musicSrcNode = null;    // MediaElementAudioSourceNode de l'élément <audio> courant
  ...
  function ensureCtx(){
    ...
    if(!AC) return null;
    ctx = new AC();
    // master pilote TOUT (musique + effets) : un seul point pour on/off + volume
    master = ctx.createGain();  master.gain.value = enabled ? volume : 0;  master.connect(ctx.destination);
    sfxGain = ctx.createGain();   sfxGain.gain.value = 0.9;  sfxGain.connect(master);
    // bus musique : la musique-fichier y est routée pour suivre le master (0.55 = musique en retrait)
  ...
  // Le niveau 3 a des variantes : choix 0 = origine, 1/2/3 = alternatives débloquées.
  const MUSIC_FILES = {
    0: ['menu.mp3','menu.ogg'],
  ...
  const LEVEL3_VARIANTS = {
    0: ['niveau3.mp3','niveau3.ogg'],        // musique d'origine
    1: ['niveau3_alt1.mp3'],
    2: ['niveau3_alt2.mp3'],
    3: ['niveau3_alt3.mp3']
  };
  let level3Choice = 0;   // variante active pour le niveau 3 (0 = origine)
  // Définit la variante de musique du niveau 3. Si le niveau 3 joue déjà, on
  // relance immédiatement avec la nouvelle piste (aperçu à chaud dans le menu).
  function setLevel3Track(choice){
    const v = (typeof choice==='number' && choice>=0 && choice<=3) ? choice : 0;
    level3Choice = v;
    if(currentTrack === 3){
      // force le rechargement de la piste 3 avec la nouvelle variante
      stopMusic();
      playMusic(3);
    }
  }
  ...
  function tryPlayFile(key){
    // Pour le niveau 3, on choisit la variante active (0 = origine, 1/2/3 = alternatives).
    const list = (key===3) ? (LEVEL3_VARIANTS[level3Choice] || LEVEL3_VARIANTS[0]) : MUSIC_FILES[key];
    if(!list) return false;
    // crée un <audio> et tente de charger la 1re source jouable
    const el = document.createElement('audio');
    el.loop = true; el.preload = 'auto';
    // Les MP3 sont servis depuis le même domaine (GitHub Pages) : pas besoin de
    // crossOrigin. On route l'élément dans le graphe Web Audio (musicGain -> master)
    // pour que le réglage on/off + volume s'applique RÉELLEMENT à la musique.
    // On laisse el.volume à 1 : c'est master.gain qui pilote le niveau final.
    el.volume = 1;
    let routed = false;
    function routeIntoGraph(){
      if(routed || !ctx || !musicGain) return;
      try{
        musicSrcNode = ctx.createMediaElementSource(el);
        musicSrcNode.connect(musicGain);
        routed = true;
      }catch(e){
        // createMediaElementSource échoue si l'élément est déjà routé ; repli :
        // on pilote alors el.volume à la main (moins propre mais fonctionnel).
  ...
  function stopMusic(){
    ...
    if(musicSrcNode){ try{ musicSrcNode.disconnect(); }catch(e){} musicSrcNode = null; }
  ...
  // ── EFFETS SONORES ──
  function sfx(name){
    if(!enabled) return;             // son coupé : aucun effet, où qu'on l'appelle
  ...
  function applyMaster(){
    const g = enabled ? volume : 0;
    if(master){
      // rampe courte pour éviter les clics, et master pilote musique + effets
      const now = ctx ? ctx.currentTime : 0;
      try{ master.gain.setTargetAtTime(g, now, 0.02); }catch(e){ master.gain.value = g; }
    }
    // Repli : si la musique n'a pas pu être routée dans le graphe (createMediaElementSource
    // a échoué), on pilote directement el.volume pour qu'elle suive quand même le réglage.
    if(audioEl && !musicSrcNode){ audioEl.volume = g; }
  }
  function setEnabled(b){
    enabled = !!b; localStorage.setItem(LS_ON, enabled?'1':'0'); ensureCtx(); applyMaster();
    // La musique continue de "tourner" (routée dans master), on ne la met en pause que
    // si on coupe le son, pour ne pas gaspiller ; on la relance à la réactivation.
    if(audioEl){
      if(!enabled){ try{audioEl.pause();}catch(e){} }
      else { try{ if(ctx&&ctx.state==='suspended') ctx.resume(); audioEl.play().then(()=>{playbackStarted=true;}).catch(()=>{}); }catch(e){} }
    }
  }
  ...
  return { unlock, playMusic, stopMusic, sfx, setEnabled, setVolume, isEnabled, volume:getVolume, setLevel3Track };
})();
