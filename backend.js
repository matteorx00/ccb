...
const Backend = (() => {
  ...
  const LS_SESSION = "ccb_session"; // <-- NOUVEAU : Clé pour mémoriser la session

  function lsGet(k){ try { return JSON.parse(localStorage.getItem(k)) || []; } catch(e){ return []; } }
  function lsSet(k,v){ localStorage.setItem(k, JSON.stringify(v)); }

  // --- NOUVEAU : Fonctions de gestion de session ---
  ...
  async function signup(name, password){
    ...
    if(useRemote){
      ...
      localStorage.setItem(LS_SESSION, name); // <-- NOUVEAU : Mémorise le joueur
      return { name };
    } else {
  ...
  // NOUVEAU : On exporte getSession et logout à la fin
  return { useRemote, signup, login, submitScore, leaderboard, leaderboardBest, globalRanking, getProgress, addProgress, myScores, getUnlocks, markLevelDone, buyCharacter, getSession, logout };
})();
