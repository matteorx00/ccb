...
const Backend = (() => {
  ...
  =================================================================== */
  const LS_UNLOCK = "ccb_unlock";   // { [player]: { spent, unlocked:[..], levels_done:[..], music3 } }
  ...
  // ── Choix de musique du niveau 3 (par joueur, stocké en local) ──
  // Valeur : 0 = musique d'origine, 1/2/3 = alternatives. Débloqué dès que le
  // niveau 3 est terminé (voir levels_done). Pas de colonne Supabase nécessaire.
  function getMusicChoice(player){
    const cur = unlLocalRead(player);
    const v = cur.music3;
    return (typeof v==='number' && v>=0 && v<=3) ? v : 0;   // défaut : origine
  }
  function setMusicChoice(player, choice){
    const v = (typeof choice==='number' && choice>=0 && choice<=3) ? choice : 0;
    unlLocalWrite(player, { music3: v });
    return v;
  }
  ...
  return { useRemote, signup, login, submitScore, leaderboard, leaderboardBest, globalRanking, getProgress, addProgress, myScores, getUnlocks, markLevelDone, buyCharacter, saveSession, loadSession, clearSession, getMusicChoice, setMusicChoice };
})();
