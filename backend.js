...
const Backend = (() => {
  ...
  =================================================================== */
  const LS_PROG = "ccb_progress";   // { [player]: { ducks, coins_l1, coins_l2, coins_l3 } }
  ...
  function progLocalRead(player){ const o=progGet(); return o[player] || { ducks:0, coins_l1:0, coins_l2:0, coins_l3:0 }; }
  ...
  // Lit { ducks, coins_l1, coins_l2, coins_l3 } pour un joueur.
  async function getProgress(player){
    if(useRemote){
      try{
        ...
        if(rows && rows.length){
          return { ducks: rows[0].ducks||0, coins_l1: rows[0].coins_l1||0, coins_l2: rows[0].coins_l2||0, coins_l3: rows[0].coins_l3||0 };
        }
        return { ducks:0, coins_l1:0, coins_l2:0, coins_l3:0 };
  ...
  async function addProgress(player, level, coinsThisRun, gained){
    ...
    if(useRemote){
      try{
        ...
        return progLocalWrite(player, { ducks: cur.ducks+gained, [col]: Math.max(cur[col]||0, coinsThisRun) });
      }
    }
    const cur = progLocalRead(player);
    return progLocalWrite(player, { ducks: cur.ducks+gained, [col]: Math.max(cur[col]||0, coinsThisRun) });
