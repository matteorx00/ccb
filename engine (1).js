...
function collideSolidsY(){
  ...
  if(THEME.ceiling && p.vy < 0){
    // barrière = bas de la roche dessinée (ceilEdgeWorld ≈ depth*0.78 + dentelure).
    // On bloque à depth*0.82 (juste sous le dessin) pour ne jamais traverser la roche.
    const ceilBottom = ceilingDepthAtWorld(p.x + p.w/2) * 0.82;
...
function checkLevelInteractions(){
  ...
  // pics de glace fixes : la POINTE tue (tout le monde, glace acérée). Zone mortelle
  // = moitié pointue du pic. dir 'up' -> pointe en haut ; 'down' -> pointe en bas.
  for(const s of iceSpikes){
    if(!aabb(p.x,p.y,p.w,p.h,s.x,s.y,s.w,s.h)) continue;
    if(s.dir==='up'){
      // stalagmite : pointe en haut (s.y). Mortel si le joueur chevauche la moitié haute.
      if(p.y+p.h > s.y && p.y < s.y+s.h*0.6){ killPlayer(); return; }
    } else {
      // stalactite : pointe en bas (s.y+s.h). Mortel si chevauche la moitié basse.
